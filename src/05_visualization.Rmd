...
plots <- lapply(categories, function(cat) {
  ...
  ggplot(df_subset, aes(x = sector, y = CO2_Reduction)) +
    ...
    geom_text_repel(aes(label = State, color = State), size = 3, max.overlaps = 10, 
                    position = position_jitter(width = 0.1, height = 0)) + # Apply jitter to labels
    scale_color_viridis_d(option = "D") +
    theme_minimal() +
    labs(
...
plots <- lapply(categories, function(cat) {
  ...
  # Generate the plot for the current category
  ggplot(df_subset, aes(y = sector, x = CO2_Reduction)) + # Swap x and y for horizontal boxes
    geom_boxplot(outlier.shape = NA, width = 0.5) +
    geom_jitter(aes(color = State), height = 0.1, alpha = 0.3) + # Adjust jitter height
    geom_text_repel(aes(label = State, color = State), size = 3, max.overlaps = 10, 
                    position = position_jitter(height = 0.1, width = 0)) + # Apply jitter to labels
    scale_color_viridis_d(option = "D") +
    theme_minimal() +
    labs(
      title = paste("Sectoral Contributions (%) for", cat, "Category"),
      y = "Sector", # Adjust labels
      x = "CO2 Emission Reductions in 2050"
    ) +
    theme(
      axis.text.y = element_text(angle = 0, hjust = 1), # Adjust text orientation
      legend.position = "none"
    )
})
